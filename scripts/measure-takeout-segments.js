#!/usr/bin/env node
/**
 * takeout-gourmet チャンネル用セグメント計測スクリプト
 *
 * VOICEVOX で台本を1セグメントずつ生成し、各セグメントの実音声長 (秒) を計測する。
 * （measure-segments.js の takeout-gourmet 版・春日部つむぎプリセット使用）
 *
 * 使い方:
 *   node scripts/measure-takeout-segments.js <script-path> [--json <out.json>] [--final-wav <wav>] [--gap-ms <ms>]
 *
 * 例:
 *   node scripts/measure-takeout-segments.js content/takeout-gourmet/scripts/reel-01-concept-v1.txt \
 *     --json content/takeout-gourmet/metadata/reel-01-concept-v1_segments.json \
 *     --final-wav content/takeout-gourmet/audio/reel_01-concept-v1.wav
 */
...
const PRESET_KEY = "takeout-gourmet-shorts";
...
if (!scriptPath) {
  console.error("Usage: node scripts/measure-takeout-segments.js <script-path> [--json <out.json>] [--final-wav <wav>] [--gap-ms <ms>]");
  process.exit(1);
}
...
const text = applyPronunciationDictionary(readFileSync(scriptPath, "utf-8"), CHANNEL_ID);
const segments = text.split(/\n\s*\n/).map((s) => s.replace(/\n/g, "").trim()).filter(Boolean);

const preset = voicevoxPresets[PRESET_KEY];
const segmentGapMs = Number.isFinite(gapMsOverride) ? gapMsOverride : preset.segmentGapMs;
const gapSec = segmentGapMs / 1000;

console.log(`Measuring ${segments.length} segments... (preset=${preset.name}, gap=${gapSec}s, speed=${preset.speedScale})`);
