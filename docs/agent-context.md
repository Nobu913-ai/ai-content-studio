# Agent Context — Multi-Channel Production Hub

This document consolidates brand strategy, production lessons, and accumulated decisions for AI coding agents (Claude Code, OpenAI Codex, etc.) working on this project.

The base content focuses on the **`genz-money` (お金の初期設定)** channel which established most of the production patterns. For channel-specific contexts (other channels), see:

- [`docs/channels/takeout-gourmet.md`](channels/takeout-gourmet.md) — Nobuグルメ (Instagram Reels 主体・テイクアウト/デリバリー)
- (`docs/channels/japanese-mindset.md`、`docs/channels/chill-culture.md` は将来必要時に作成)

When agents work on a specific channel, they should read both this file (general patterns) **and** the channel-specific context file.
...
### ナレーション同期・段階表示ルール (Shorts #03 で確定)

Shorts #03 複利回では、最終品質の差分は「デザインの豪華さ」より **ナレーションと画面理解の同期** で決まった。以後の Remotion 制作では以下を標準ルールにする。

- **画面内テキストは、ナレーション/字幕で読まれる文か、図解に必要な短いラベルだけにする**。ナレーションにない結論文は、固定コメント・プロフィール・note側に逃がす。
- **ナレーションで触れる前に答えを全部見せない**。図表・内訳・チェックリストは narration の順番に合わせて staged reveal する。
- **説明した数字は、画面上で指し示す**。グラフの場合は、該当年の目盛・点ラベル・補助線・下部カードの強調を同期させる。
- **丸め方を統一する**。「約」と言う場合は視聴者が追いやすい区切り値にする (例: 2,507万円ではなく約2,500万円)。
- **内訳は比較図にしない**。元本と運用益のような part-whole は `VS` ではなく、積み上げ・ゲージ・「A + B = C」で示す。
- **説明終了後に 0.5-0.8秒程度ホールドする**。すぐ切り替えると視線が追いつかない。
- **強調アニメーションは控えめに一度だけ**。pulse / glow / typing が強すぎると説明を邪魔するため、scale と opacity の小さな変化を基本にする。
- **同じ役割の文字サイズ・枠線・余白は共通化する**。画面ごとに太さやサイズが変わると量産感・違和感が出る。
...
- **Shorts #03 複利回 v5 公開品質まで改善**:
  - 2,500万円の内訳を `1,080万円 + 約1,420万円` として part-whole で明示
  - グラフは 10年 / 20年 / 30年のナレーション順に点・補助線・下部カードを段階表示
  - 画面内の数字をナレーションの丸め値に統一 (`約2,500万円`)
  - ナレーションにない補足文を削除し、画面情報量を抑制
  - pulse / glow / typing を控えめにし、グラフ線・ゲージの動きを滑らかに調整
- **制作ルール更新**: 「ナレーション同期・段階表示ルール」を本ドキュメントと `docs/shorts-production-workflow.md` に追加
- **lint強化**: `scripts/lint-shot-plan.js` に `footerLabel` / `loopLabel` が narration / captionSegments に接続していない場合の警告を追加
...
- [ ] Shorts #03 複利回の公開準備 (SEO/repurpose/handoff + note導線確認)
...
- [ ] Shorts #04 制作 (テーマ: 家計管理 or ポイ活 or ふるさと納税 等)
